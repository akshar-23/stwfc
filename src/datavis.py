...
import pandas as pd
import json
from use_coac import board_to_str
...
def process(d_results):
    ...
    for game in os.listdir(d_results):
        ...
        for run in os.listdir(d_game):
            d_run = f"{d_game}/{run}"
            if os.path.isfile(d_run) or not os.path.exists(f"{d_run}/stats.json"):
                continue
            ...
            for lvl in os.listdir(d_run):
                i = lvl[0]
                d_lvl = f"{d_run}/{lvl}"
                if os.path.isfile(d_lvl) or not os.path.exists(f"{d_lvl}/init_raw.json") or not os.path.exists(f"{d_lvl}/final_raw.json"):
                    continue
                ...
                if np.any(final_lvl == ""):
                    ...
                    sum_non_blank = sum([value for key, value in value_counts.items() if key != "_"])
                    char_counts[f"{i}"] = {k: f"{v}" for k, v in value_counts.items()}
                    lvl_density = sum_non_blank/sum_counts
                    density[f"{i}"] = f"{lvl_density}"
                    if value_counts["P"] > 1:
            ...
            if num_success > 0:
                ...
                stats["density_non_blank"] = density
        ...
        game_pct_trivial = 0
        game_pct_has_extra_P = 0
        if game_num_success > 0:
        ...
        game_stats = {
            ...
            "game_pct_trivial": game_pct_trivial,
            "game_num_has_extra_P": game_num_has_extra_P,
            "game_pct_has_extra_P": game_pct_has_extra_P,
        }
...
if __name__ == "__main__":
    ...
    results_folder = f"../AIIDE/results"
